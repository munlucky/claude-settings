...
`node scripts/doctor.mjs check --json` emits `schemaVersion: moonshot-doctor-readiness.v1` and keeps the existing CLI flags for source and explicit payload-root checks. Optional `--evidence-root <root>` defaults to the repository root and lets an installed-payload check read lab, eval, and research readiness from a preserved external closeout tree without moving generated state into the installed payload. The readiness schema records `repoRoot` and `evidenceRoot` and includes `runtimeSurface`, `skillsLock`, `labReadiness`, `evalReadiness`, `researchReadiness`, `profileTrust`, and `generatedStateBoundary`.
