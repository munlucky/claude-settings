...
|---|---|---|---|---|
| 01 | Public Surface and Skill Skeleton | `01-public-surface-skill-skeleton-v1.ko.md` | - | no |
| 02 | Architecture Artifact Templates and Schemas | `02-architecture-artifact-templates-schemas-v1.ko.md` | 01 | partial |
| 03 | Supporting Internal Skills | `03-supporting-internal-skills-v1.ko.md` | 01 | partial |
| 04 | Architecture Context Builder | `04-architecture-context-builder-v1.ko.md` | 02, 03 | no |
| 05 | Greenfield PRD Flow | `05-greenfield-prd-flow-v1.ko.md` | 02, 03, 04 | partial |
| 06 | Brownfield Codebase Flow | `06-brownfield-codebase-flow-v1.ko.md` | 02, 03, 04 | partial |
| 07 | Product and Execution Integration | `07-product-execution-integration-v1.ko.md` | 05, 06 | no |
| 08 | Regression and Evaluation Gates | `08-regression-evaluation-gates-v1.ko.md` | 07 | no |

## Parallel Execution Plan

| Wave | Phases | Eligibility | Blockers / Notes |
|---|---|---|---|
| wave-0 | 01 | sequential | public runtime surface and skill skeleton anchor later phases |
| wave-1 | 02, 03 | parallel after 01 | disjoint `templates/schemas/scripts` vs `skills/internal`; both may read docs/tests |
| wave-2 | 04 | sequential | depends on schemas/templates and supporting skill contracts |
| wave-3 | 05, 06 | parallel after 04 | greenfield and brownfield fixtures should use disjoint fixture roots |
| wave-4 | 07 | sequential | mutates shared skill docs, README, runtime docs, and adoption boundary |
| wave-5 | 08 | sequential | final regression/eval gate and completion authority evidence |

## Source Traceability Matrix

| Req ID | Source | Requirement Summary | Phase | Acceptance Evidence |
|---|---|---|---|---|
| MSA-REQ-01 | Skill Surface Design | Add `moonshot-architecture` as public runtime skill | 01 | allowlist test, package dry-run |
| MSA-REQ-02 | Required New Files / Templates | Contract-backed architecture artifacts | 02 | template/schema tests, artifact validator |
| MSA-REQ-03 | Skill Surface Design / Internal Supporting Skills | Add internal stage-owner skills without public exposure | 03 | skill contract review, runtime-surface negative assertion |
| MSA-REQ-04 | Context Pack Design | Build prompt-safe architecture context | 04 | context-pack tests and leakage negative tests |
| MSA-REQ-05 | Greenfield PRD Mode | Produce Greenfield architecture package | 05 | greenfield fixture validation and traceability matrix |
| MSA-REQ-06 | Brownfield Codebase Mode | Produce Brownfield recovery, fit-gap, migration package | 06 | brownfield fixture validation and repository-evidence assertions |
| MSA-REQ-07 | Handoff Contracts | Integrate product, plan-writer, orchestrator, phase-runner handoff | 07 | workflow-e2e contract tests |
| MSA-REQ-08 | Regression and Evaluation Gates | Add quality and safety regression protection | 08 | `npm test`, `npm run test:package`, `npm run test:eval`, dry-run installer/package |

## Closeout Authority

- Phase-local completion requires the active phase doc checklist, fresh verification command output, QA/SCORECARD/HANDOFF evidence under the configured execution root, and parent review acceptance.
- Whole-plan completion is not established by `phase-status.yaml` alone. After Phase 08, record fresh verification-plane evidence and run `node scripts/runtime-state.mjs assess-completion --json`; clean completion requires an accepted DB decision.
- Required closeout evidence includes `unit`, `package`, `eval`, `installer`, `browser`, `security`, and `quality`. If a plane is not applicable, the closeout evidence must say so explicitly rather than omitting it.
- If shared runtime files or package payload change, account-root install sync is a separate closeout item and must not be implied by source tests.
