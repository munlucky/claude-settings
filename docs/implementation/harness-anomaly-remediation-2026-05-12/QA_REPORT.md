...
| Runtime namespace mismatch UX | `.claude/scripts/runtime-cli.sh`, `.claude/scripts/verify-phase-runtime-parity-shell-core.sh`, `.claude/scripts/lib/runtime-parity-classifier.mjs`, `.claude/scripts/lib/runtime-parity-classifier.test.mjs` | Classify WSL loading the Windows npm Codex shim as `runtime_namespace_mismatch`, prefer WSL user-local npm binaries during runtime prep, and avoid treating this as an implementation failure. | `node --test .claude/scripts/lib/runtime-parity-classifier.test.mjs`; `bash -n .claude/scripts/runtime-cli.sh`; `bash -n .claude/scripts/verify-phase-runtime-parity-shell-core.sh`; optional/required runtime parity smoke |
