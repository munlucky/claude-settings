...
const PUBLIC = ['product-orchestrator', 'moonshot-architecture', 'moonshot-orchestrator', 'moonshot-phase-runner', 'moonshot-plan-writer', 'commit-moonshot', 'session-logger', 'explain-diff-html'];
...
const sourceAt = async (root, ref, relative) => ref
  ? (() => {
      try {
        ...
        const isNewWorkingTreePath = details.includes(`path '${relative}'`) && details.includes('not in');
        if (!isNewWorkingTreePath) throw error;
        // A historical baseline may predate a newly exposed public skill. Use
        // the current source for that entry while retaining baseline contents
        // for every skill that existed at the selected ref.
...
async function score(args) {
  ...
  const result = {
    ...
    requiredRunsPerCase: 3, callsUsed: 6, caseCountPerVariant: 120, scorerVersion: 'moonshot-model-skill-scorer.v3',
