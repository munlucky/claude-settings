...
test('Kernel launch delegates to the native provider and keeps managed runtime Node-only', async () => {
  ...
  assert.equal(spec.providerRuntime.executionLayer, 'shared-host-dispatch');
  assert.equal(spec.providerRuntime.trackIsolation, 'profile-and-data-root');
  ...
  assert.equal(relaySpec.providerRuntime.executionLayer, 'shared-host-dispatch');
