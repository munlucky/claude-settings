...
test('evaluateOntologyConstraints detects never and ask_first violations', async () => {
  ...
  await writeAtomicJsonl(path.join(root, 'knowledge', 'ontology', 'constraints.jsonl'), [
    { id: 'c1', projectId: 'proj-ont', type: 'ontology_constraint', statement: 'Never mutate Relay state', pattern: 'Never mutate Relay state', severity: 'never', scope: ['scripts/kernel/**'], status: 'verified', trustTier: 'verified', createdAt: '2026-07-23T00:00:00.000Z' },
  ]);

  const evalRes = await evaluateOntologyConstraints({ projectId: 'proj-ont', paths: ['scripts/kernel/state.mjs'], statements: ['Never mutate Relay state'], env });
