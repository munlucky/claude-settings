...
description: 프로젝트 승격 후보를 검토하고 승인된 범용 하네스 지식을 claude-settings MemoryGraph에 저장합니다.
---
...
- 반드시 하네스 저장소 루트, 일반적으로 `C:\dev\claude-settings`에서 실행합니다.
- `context.project_id: claude-settings`로만 저장합니다.
...
2. 현재 프로젝트 id가 `claude-settings`인지 확인합니다.
...
- `claude-settings` 밖에서 실행: `status: failed`를 반환하고 쓰지 않습니다.
