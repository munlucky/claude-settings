...
strict 실행에서는 Finish / Handoff로 넘어가기 전에 Verify stage를 닫는 마지막 게이트다.
...
2. strict면 최신 증거가 필요하다.
   ...
   - verifier artifact가 있으면 자유형 notes보다 그 구조화된 증거를 우선한다
...
- 구조화된 `verdict.workflowEvidence` 경고가 있으면 수동 해석보다 그것을 우선 신뢰한다.
- contract 기반 검증에서 최신 증거 없는 pass 상태는 여전히 차단한다.
- 코드 변경 마감에서 review/finish workflow evidence가 없으면 이를 선택적 메타데이터가 아니라 차단 사유로 본다.
