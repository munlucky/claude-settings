...
각 단계 완료 시 진행 상태를 명확히 출력합니다.

### 분석 단계 (Phase: Analysis)
```
📊 PM 분석 진행 상태
├─ [1/5] ✅ 작업 분류 완료 (taskType: {taskType})
├─ [2/5] ✅ 복잡도 평가 완료 (complexity: {complexity})
├─ [3/5] 🔄 불확실성 검출 중...
├─ [4/5] ⏳ 시퀀스 결정 대기
└─ [5/5] ⏳ 실행 체인 구성 대기
```

### 시퀀스 결정 후 (Phase: Chain Execution)
```
🔗 실행 체인 진행 상태 ({phase})
├─ [1/N] ✅ pre-flight-check 완료
├─ [2/N] 🔄 requirements-analyzer 실행 중...
├─ [3/N] ⏳ context-builder 대기
└─ [.../N] ⏳ 나머지 단계 대기
```
