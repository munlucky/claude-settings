...
const ENVIRONMENT_PATTERNS = [
  ...
  { code: 'mcp_cleanup_eperm', test: /(?:^|\b)(?:mcp cleanup|cleanup mcp|kill mcp process group|process group cleanup|tree cleanup)(?:\b|:).*(ep?erm|eacces|permission denied|access is denied)/i },
  { code: 'path_update_denied', test: /(?:^|\b)(?:path update|update path|prepend path|PATH)(?:\b|:).*(denied|permission denied|access is denied|ep?erm|eacces|readonly|read only)/i },
