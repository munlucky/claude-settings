...
map_codex_effort_profile() {
    case "${1:-standard}" in
        economy) printf '%s\n' "low" ;;
        standard) printf '%s\n' "medium" ;;
        deep) printf '%s\n' "high" ;;
        max) printf '%s\n' "${HARNESS_CODEX_MAX_REASONING_EFFORT:-xhigh}" ;;
        *) printf '%s\n' "medium" ;;
    esac
}

EFFORT_PROFILE="${AGENT_LOOP_EFFORT_PROFILE:-${MOONSHOT_EFFORT_PROFILE:-standard}}"
CODEX_REASONING_EFFORT="${AGENT_LOOP_CODEX_REASONING_EFFORT:-${MOONSHOT_CODEX_REASONING_EFFORT:-$(map_codex_effort_profile "$EFFORT_PROFILE")}}"
MODEL_ROUTE_PROVIDER=""
MODEL_ROUTE_MODEL=""
MODEL_ROUTE_EFFORT=""
MODEL_ROUTE_REASONING_CONTROL=""
MODEL_ROUTE_PROFILE="$EFFORT_PROFILE"
while IFS='=' read -r key value; do
    case "$key" in
        MODEL_ROUTE_PROVIDER) MODEL_ROUTE_PROVIDER="$value" ;;
        MODEL_ROUTE_MODEL) MODEL_ROUTE_MODEL="$value" ;;
        MODEL_ROUTE_EFFORT) MODEL_ROUTE_EFFORT="$value" ;;
        MODEL_ROUTE_REASONING_CONTROL) MODEL_ROUTE_REASONING_CONTROL="$value" ;;
        MODEL_ROUTE_PROFILE) MODEL_ROUTE_PROFILE="$value" ;;
    esac
done < <(node "$SCRIPT_DIR/lib/model-routing-policy.mjs" env "$RUNNER_RUNTIME" "${PHASE_MODEL_STAGE:-phase_implementation}" "$EFFORT_PROFILE" 2>/dev/null || true)
CODEX_MODEL="${AGENT_LOOP_CODEX_MODEL:-${MOONSHOT_CODEX_MODEL:-$MODEL_ROUTE_MODEL}}"
CLAUDE_MODEL="${AGENT_LOOP_CLAUDE_MODEL:-${MOONSHOT_CLAUDE_MODEL:-$MODEL_ROUTE_MODEL}}"
CLAUDE_EFFORT="${AGENT_LOOP_CLAUDE_EFFORT:-${MOONSHOT_CLAUDE_EFFORT:-$MODEL_ROUTE_EFFORT}}"
if [[ -n "$MODEL_ROUTE_EFFORT" ]]; then
    CODEX_REASONING_EFFORT="${AGENT_LOOP_CODEX_REASONING_EFFORT:-${MOONSHOT_CODEX_REASONING_EFFORT:-$MODEL_ROUTE_EFFORT}}"
fi
