...
For the Codex `/` command surface, the Kernel default is an explicit direct profile operation: `node bin/moon-relay-kernel.mjs profile-install --runtime codex --account-root`. It installs the Kernel command-skill set at the Codex account root, moves known Moonshot Relay workflow skills into a recoverable backup, and leaves unselected ordinary Codex tasks outside Kernel.
