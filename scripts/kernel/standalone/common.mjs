...
import { resolveKernelRuntimeHome, readProjectTrackSync } from '../runtime-home.mjs';
...
export function resolveStandaloneProject({ cwd = process.cwd(), env = process.env } = {}) {
  const track = readProjectTrackSync(cwd, { env });
  if (track !== 'kernel') throw Object.assign(new Error(`wrong_harness: standalone Kernel utility requires project track=kernel (found ${track || 'none'})`), { code: 'wrong_harness' });
