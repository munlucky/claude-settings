...
14. closeout gate가 유용한 implementation evidence 이후 phase를 reject하면 `record-eval-result --regression-worsened true` 또는 blocking runtime event로 기록하고, finding을 carry-forward state에 남긴 뒤 다음 independent actionable phase를 계속합니다.
15. 전체 plan directory가 구현될 때까지 다음 actionable phase로 계속 진행합니다. 최종 whole-plan completion claim만 `assess-completion`의 `accepted`가 필요합니다.
