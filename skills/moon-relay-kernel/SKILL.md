...
6. When `kernel next` returns an independent review action (for security review, protected obligations, T3 judgments, or explicit requirements), satisfy it with a review receipt recorded from an independent reviewer session; Host review fallback transports resolve automatically without manual switching. Ordinary tasks use the current owner directly and do not require an independent reviewer.
