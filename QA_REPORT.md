...
- Date: 2026-07-29
- Scope: Kernel — prompt cache-hit optimization plus Claude Opus 5 / Codex GPT-5.6 provider execution profiles (Waves 0–8 of the work guide). Branch `feat/kernel-cache-multi-provider-optimization`, baseline `016afe1d`.
- Added deterministic context segments (`scripts/kernel/context-segments.mjs`) splitting the prompt by rate of change — host-stable, project-stable, run-stable, volatile — so identical state renders identical bytes regardless of SQLite row order or observation timestamps. The legacy `promptBlock` is rebuilt from the segments and stays compatible.
- Added the Host prompt envelope (`prompt-envelope.mjs`, `model-capsule-view.mjs`, `provider-prompt-policy.mjs`). Common and provider policy are separate segments, so a Claude revision cannot invalidate Codex's warm prefix. Run/step/capsule/admission ids live in `control` and never reach a cacheable segment; `resolvedModel`/`resolvedEffort` are cache identity but not prompt text.
- Consolidated the shared execution contract once (`prompts/common-execution.mjs`) with conditional planning, verification order, and a default-deny delegation gate (`common-model-policy.mjs`, `maxNestedAgents` 0). Provider profiles carry only provider-specific policy: Claude thinking/effort/cache breakpoints/output-artifact guard; Codex Sol-Terra-Luna routing, isolated `<profile>.config.toml` overlays under the Kernel runtime home, AGENTS.md content policy, subagent/Ultra/Fast-mode/Responses-API capability gates.
- Extended usage receipts with cache read/write, model/effort/speed, reasoning context, and session lineage, via an additive SQLite migration and additive schema properties. Unmeasured values persist as `NULL`, and `cachedInputTokens` stays populated in both directions for existing readers.
- **Wave 1 found nothing to remove.** Every Kernel-loaded prompt surface was searched for the guide's English and Korean legacy patterns; all repository matches were in Relay-mode skills the Kernel Host does not load. Nothing was deleted on suspicion. The finding, its method, and its scope are recorded in `artifacts/kernel-prompt-audit/`, and `tests/kernel-prompt-legacy-instruction.test.mjs` now fails if any pattern is reintroduced.
- **Two defects found in existing code while testing.** (1) `schemas/kernel.model-usage.schema.json` is `additionalProperties: false` but never declared `capsuleId`, `capsuleDigest`, `admissionId`, `admissionDigest`, or `stepId`, so any capsuled receipt would fail validation; the properties are now declared. (2) `tests/kernel-model-routing-e2e.test.mjs` is flaky under a nested Kernel proof — its inner run intermittently reports `verification command mutated tracked source`. It failed once in five executions of the same command and is left open, not worked around.
- Verification evidence (Kernel-executed): `npm run test:active:2` 659/659 including 51 new test files, `npm run test:routing` 33/33. Also measured directly: `npm run test:package` 240/240, `npm run test:kernel` 719/720, `npm run test:active:1` 612/613, cache replay corpus 16/16.
- **Open, requires an operator decision.** The declared acceptance criterion AC-19 ("`npm run test:kernel` and `npm test` pass in full") is not satisfied and was not satisfied at the baseline either. `tests/kernel-path-canonicalization.test.mjs` fails with `EPERM` because creating a symlink on this Windows host needs Developer Mode or elevation, and the browser-flow case in `tests/browser-scenario-contract.test.mjs` fails for a missing runner. Both reproduce unchanged at `016afe1d`. No regression was introduced — every suite matches or exceeds its baseline count — but the Kernel completion gate `acceptanceCovered` stays unmet because AC-19 is claimed by nothing. Coverage was declared for AC-1 through AC-18 only; AC-19 was deliberately not claimed rather than asserted on evidence that does not exist. The run is therefore parked at `PROVE` with `finalizationStatus: incomplete_gates` pending a decision to amend AC-19 or to fix the two environmental failures.
- Also refreshed `package/harness-surface-budget.json` baseline to the measured totals (+87 files, +6,058 non-blank lines) for the source and tests this change genuinely adds. `allowedDelta` was left untouched so the drift gate keeps working.

- Date: 2026-07-27 (4)
- Scope: Kernel — three further defects from automated review on PR #16 (second round). All three were real.
- P1 — **any frontier turn could mint a review receipt.** `assertIndependentReviewSession()` checked the model class, the enforcement status, and session independence, but never that the turn was routed AS a review. A planner turn at T3 (frontier class, its own session, enforced routing) satisfied all three, so `recordReview()` would mint a trusted receipt from it and complete `security-review` without a reviewer ever running. The decision's `role` must now be `reviewer` and its `actionKind` a review action.
- P1 — **a replacement plan reusing a declared step id vanished.** `run_steps` is keyed `(run_id, step_id)` and `createRunSteps()` upserts with `DO NOTHING`, so a replan that reused a custom id silently dropped the replacement — and a plan that lost every step left `allStepsPassed([])` reading the run as settled. Three changes: the store now throws `STEP_ID_COLLISION` on a cross-revision id clash instead of dropping the row; the planner qualifies a reused declared id with the plan revision (`auth-slice@r2`) and re-resolves the dependency chain against the final ids; and `allStepsPassed` treats "steps exist but none at this revision" as unsettled rather than finished.
- P2 — **write sets were compared case-sensitively.** `matchPathScope()` lowercases both sides, so `src/Auth/**` and `src/auth/**` authorise the same files, but the Safe Wave conflict check called them disjoint and would have dispatched racing workers. The comparison now mirrors the matcher: separator- and case-insensitive, via an exported `scopesOverlap()`.
- Verification evidence: `npm run test:kernel` 422/422, `npm run test:routing` 33/33, sentinel 25 cases with 0 false completions. Six regression tests: a planner turn refused as a review, the decision-level role rule, a replan reusing both step ids, the empty-revision plan, the store-level collision, and the case-insensitive scope comparison.
